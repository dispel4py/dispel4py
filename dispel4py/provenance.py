...
def getDestination_prov(self, data):
    if 'TriggeredByProcessIterationID' in data[self.input_name][0]:
        output = tuple([data[self.input_name][0]['data'][x] for x in self.groupby])
    else:
...
class ProvenancePE(GenericPE):
    ...
    def pe_init(self,*args,**kwargs):
        ...
        self.instanceId = self.name+"-Instance-"+socket.gethostname()+"-"+getUniqueId()
        ...
        try:
            ...
            try:
                if self.provon==True:
                    ...
                    super(ProvenancePE, self).write(OUTPUT_METADATA,trace['metadata'])
            except:
    ...
    def initParameters(self):
        ...
        self.outputdest=self.controlParameters['outputdest'] if 'outputdest' in self.controlParameters else 'None'
        self.rootpath=self.controlParameters['inputrootpath'] if 'inputrootpath' in self.controlParameters else 'None'
        self.outputid=self.controlParameters['outputid'] if 'outputid' in self.controlParameters else 'None'
    ...
    def wirteResults(self,name,result):
          ...
          if type(result)==dict  and '_d4p_prov' in result:
              ...
              if 'error' in meta:
                  ...
                  self.extractProvenance(result,error=self.error,output_port=name,**meta)
    ...
    def __computewrapper(self,inputs):
        ...
        try:
            ...
            if self.impcls!=None and isinstance(self, self.impcls):
    ...
    def packageAll(self,contentmeta):
        ...
        if self.provon:
            try:
                ...
                metadata.update({'parameters':self.dicToKeyVal(self.parameters)})
        ...
        output={"metadata":metadata,"error":self.error,"pid":"%s" % (os.getpid(),)}
#        self.log("PACKAGED "+str(output))
    ...
    """
    def extractProvenance(self,data, location="", format="", metadata={},control={},attributes={},error="",output_port="output"):
        ...
        if 'con:skip' in control and control['con:skip']==True:
            ...
            self.provon=True
            usermeta=self.buildUserMetadata(data,location=location, format=format, metadata=metadata,control=control,attributes=attributes,error=error,output_port=output_port)
    ...
    def getMetadata(self,data):
        ...
        if type(streammeta) != list:
            ...
            streammeta=self.streamItemsMeta[str(id(data))] if type(self.streamItemsMeta[str(id(data))])==list else [self.streamItemsMeta[str(id(data))]]

        elif type(streammeta)==list:

            try:

                if type(self.streamItemsMeta[str(id(data))])==list:
                            streammeta=streammeta+self.streamItemsMeta[str(id(data))]
                if type(self.streamItemsMeta[str(id(data))])==dict:
        ...
        if (self.streamItemsLocations!={}):
                streamItem.update({"location": self.streamItemsLocations[str(id(data))]})
        if (self.streamItemsFormat!={}):
                streamItem.update({"format": self.streamItemsFormat[str(id(data))]})
        streamlist.append(streamItem)
    ...
    def buildUserMetadata(self,data,**kwargs):
        ...
        if type(streammeta) != list:
            ...
            streammeta=kwargs['metadata'] if type(kwargs['metadata'])==list else [kwargs['metadata']]

        elif type(streammeta)==list:

            try:

                if type(kwargs['metadata'])==list:
    ...
    def buildDerivation(self,data,port=""):
        ...
        try:
            ...
            derivation={'port':port,'DerivedFromDatasetID':data['id'],'TriggeredByProcessIterationID':data['TriggeredByProcessIterationID']}
...
def injectProv(object,provType,active=True,**kwargs):
    ...
    if type(object)==WorkflowGraph:
        object.flatten()
        nodelist=object.getContainedObjects()
        for x in nodelist:
            injectProv(x,provType,**kwargs)
    else:    
        print "Injecting provenance to: "+object.name+" Original type: "+str(object.__class__.__bases__)
        ...
        object.__class__ = type(str(object.__class__),(provType,object.__class__),{})
        
        print "Injecting provenance to: "+object.name+" Transoformed: "+str(type(object))
...
' This methods enriches the graph to enable the production and recording of run-specific provenance information '
' the provRecorderClass parameter can be used to attached several implementatin of ProvenanceRecorder which could dump to files, dbs, external services, enrich the metadata, etc..'

def InitiateNewRun(graph, provRecorderClass, provImpClass=ProvenancePE, input=[],username=None,workflowId=None,description="",system_id=None,workflowName=None,w3c_prov=False,runId=None):
        
        
        if username==None or workflowId==None or workflowName==None:
        ...
        attachProvenanceRecorderPE(graph,provRecorderClass,runId,username,w3c_prov)

        return runId
    
def attachProvenanceRecorderPE(graph,provRecorderClass,runId=None,username=None,w3c_prov=False):
        partitions=[]
        nopartitions=False
        try:
        ...
        if username==None or runId==None:
            raise Exception("Missing values")
        graph.flatten()
        ...
        for x in nodelist:
            if isinstance(x, (WorkflowGraph)):
                attachProvenancePE(x,provRecorderClass,runId=runId,username=username,w3c_prov=w3c_prov)
            
            if isinstance(x, (ProvenancePE)) and x.provon==True:
                provrecorder=provRecorderClass(toW3C=w3c_prov)
                #provrecorder.numprocesses=1
                x.controlParameters["runId"]=runId
                x.controlParameters["username"]=username
                graph.connect(x,OUTPUT_METADATA,provrecorder,provrecorder.INPUT_NAME)
...
class ProvenanceSimpleFunctionPE(ProvenancePE):
    ...
    def __init__(self,*args,**kwargs):
        ...
        self.__class__ = type(str(self.__class__),(self.__class__,SimpleFunctionPE),{})
...
class NewWorkflowRun(GenericPE):
    ...
    def makeRunMetdataBundle(self,input=[],username=None,workflowId=None,description="",system_id=None,workflowName=None,w3c=False,runId=None):
    ...
    def _process(self,inputs):
        ...
        bundle=self.makeRunMetdataBundle(username=self.parameters["username"],input=self.parameters["input"],workflowId=self.parameters["workflowId"],description=self.parameters["description"],system_id=self.parameters["system_id"],workflowName=self.parameters["workflowName"],runId=self.parameters["runId"])
...
class ProvenanceRecorderToFile(ProvenanceRecorder):
    ...
    def __init__(self, name='ProvenanceRecorderToFile',toW3C=False):
        ...
        self.inputconnections[ProvenanceRecorder.INPUT_NAME] = { "name" : ProvenanceRecorder.INPUT_NAME }
    ...
    def process(self, inputs):
        ...
        if type(prov)==list and "data" in prov[0]:
            
            prov=prov[0]["data"]
            
        
            
        if self.convertToW3C==True:
            out=toW3Cprov(prov)
        else:
            out=prov   
...
class ProvenanceRecorderToService(ProvenanceRecorder):
    ...
    def _process(self, inputs):
        ...
        self.connection=httplib.HTTPConnection("verce-portal-dev.scai.fraunhofer.de")
        ...
        self.connection.request("POST", "/j2ep-1.0/prov/workflow/insert", params, headers)
...
class ProvenanceRecorderToServiceBulk(ProvenanceRecorder):
    ...
    def postprocess(self):
       ...
       self.connection.request("POST", "/j2ep-1.0/prov/workflow/insert", params, headers)
       response = self.connection.getresponse()
       self.log("Postprocress: "+str((response.status, response.reason, response, response.read())))
       self.connection.close()
       self.bulk=[]
    ...
    def _process(self, inputs):
        ...
        if len(self.bulk)==15:
            ...
            self.connection.request("POST", "/j2ep-1.0/prov/workflow/insert", params, headers)
            response = self.connection.getresponse()
            self.log("progress: "+str((response.status, response.reason, response, response.read())))
            self.connection.close()
            self.bulk=[]
