...
class StreamProducer(SeismoPE):
    ...
    def compute(self):
        ...
        The input data is available as "self.st" or, if the input data is a
        list, it is stored in "self.streams".
        In this example, there is only one piece of input data, the filename,
        and it is stored in "self.st".
        Compute is called for each input block, so if there are n filenames
